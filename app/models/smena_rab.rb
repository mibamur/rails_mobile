class SmenaRab < ActiveRecord::Base
  ...
  def self.import(file)
    ...
    (2..spreadsheet.last_row).each do |i|
      ...
      smena_rab = find_by_id(row["id"]) || new
      smena_rab.update row.to_hash
      smena_rab.save!
    end
  end
  ...
  def self.to_csv(options = {})
    CSV.generate(options) do |csv|
      csv << column_names
      all.each do |smena_rab|
        csv << smena_rab.attributes.values_at(*column_names)
      end
    end
  end
